	...
	jQuery.fn.beautifully_steps = function(fn_options){
		...
		self.active_step = function(step_number){
			...
			if (step_number <= self.step_div.length){
				...
				if (self.options.animate_updte_step){
					...
					self.step_div[n].css({'display': 'inlline-block'});
				}
			}
		};
		self.hint_notes = new Array(), self.count = 0;
		self.addHintNote = function(key, text, mainItem)
		{
			if (!(mainItem instanceof jQuery)){
				mainItem = jQuery(mainItem);
			}
			self.hint_notes[self.count++] = {
				'key': key,
				'text': text,
				'mainItem': mainItem
			};
				...
				for(n = 0; n < self.hint_notes.length; n++){
					if (self.hint_notes[n].mainItem instanceof jQuery && self.hint_notes[n].mainItem.length > 0){
						...
						_box_note.css({
							'display': 'none',
							'position': 'absolute',
							'left': self.hint_notes[n].mainItem.offset().left +  self.step_div[n].innerWidth()/2-64,
						...
						var _box_header_child = jQuery('<div class="icon-hint-arrow-down-hide" />');
						...
						if (self.hint_notes[n].mainItem.offset().left + 250 > jQuery(window).width()){
							_box_conteiner.css({
								'left': -(self.hint_notes[n].mainItem.offset().left + 250 - jQuery(window).width() - 65)
							});
						}
		...
		self.showHintItem = function(n){
			setTimeout(function(){
				if (self.hint_notes[n-1] != undefined){
					var _box_note = jQuery('#hint-note-'+self.hint_notes[n-1].key);
					_box_note.css({
						'left': self.hint_notes[n-1].mainItem.offset().left +  self.step_div[n-1].innerWidth()/2-64,
						'top' : self.hint_notes[n-1].mainItem.offset().top  - self.subTop,
						'display' : 'block'
					});
				}
			}, 800);
